					...
					if(count($BuildingBlockCategory) > 0)
						...
						foreach($BuildingBlockCategory as $BuildingBlocks)
							...
							if($First_Building_Block==1)
								...
								$PageBody .= '<div style="width: 80%; max-width: 680px; height: 25px; margin-left: 8px; margin-top:10px; background-color: ' . $category_hex . '; position: absolute; z-index: 50;" id="line-hex"></div>' . chr(10);
								...
								if($Next_Page!='')
									{
									$PageBody .= '<a href="' . $Next_Page . '" class="newer">Next >></a>' . chr(10);
									}
								else
									{
									$PageBody .= '<a href="#" class="newer"> </a>' . chr(10);
									}	
								$PageBody .= '</div>' . chr(10);						
			
								$page_content = $PageHeader . chr(10) . $PageBody . chr(10) . $ReturnHTML;
			
								$write_page_folder = "journey/complete/";
